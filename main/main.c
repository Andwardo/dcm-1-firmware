...
void app_main(void) {
    ESP_LOGI(TAG, ">> DCM-1 v8.0.9 booting...");
    ...
    ESP_LOGI(TAG, "[8] Wi-Fi ready (mode=%s)", wifi_manager_is_sta_connected() ? "STA" : "AP");
    httpd_handle_t server = start_http_server();
    if (!server) {
        ESP_LOGE(TAG, "[!] HTTP server failed to start");
    }
    for (;;) {
        // UPDATED: Using PRIu32 for portable 32-bit unsigned int printing.
        ESP_LOGI(TAG, "[9] Heartbeat @ %" PRIu32 " ms", xTaskGetTickCount() * portTICK_PERIOD_MS);
